...
```{r spotify_connect, warning=FALSE, echo=FALSE} 
library(httr)
library(yaml)
...
```{r get_top_hits, warning=FALSE, echo=FALSE}
...
```{r display_simple, warning=FALSE, echo=FALSE}
...
```{r get_repeats, message=FALSE, echo=FALSE, warning=FALSE}
library(dplyr)
...
```{r get_more_data, warning=FALSE, echo=FALSE}
